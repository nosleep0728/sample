...
OffsetX := 20 ;offset in caret position in X axis
;OffsetY := 20 ;offset from caret position in Y axis
OffsetY := 0 ;offset from caret position in Y axis
;BoxHeight := 165 ;height of the suggestions box in pixels
BoxHeight := 200 ;height of the suggestions box in pixels
...
OtherKeyList := "-" ;list of key names separated by `n that make up words
ResetKeyList := "Esc`nSpace`nHome`nPGUP`nPGDN`nEnd`nLeft`nRight`nRButton`nMButton`n,`n.`n/`n[`n]`n;`n\`n=`n```n""`n'"  ;list of key names separated by `n that cause suggestions to reset
ResetKeyList2 := "Esc"  ; GUI만 없애기 위함. (원래의 동작을 하지 않도록 처리함.)
;ResetKeyList := "Esc`nSpace`nHome`nPGUP`nPGDN`nEnd`nLeft`nRight`nRButton`nMButton`n,`n/`n[`n]`n;`n\`n=`n```n"""  ;list of key names separated by `n that cause suggestions to reset
...
SetHotkeys(NormalKeyList,NumberKeyList,OtherKeyList,ResetKeyList,TriggerKeyList,ResetKeyList2)
...
if( SubStr(NewWord,1,4) = "row." ){
	NewWord := SubStr(NewWord,5)
	NewWord := RegExReplace(NewWord,"\s+.*","")
}
if( SubStr(NewWord,1,4) = "nxc." ){		; 공백이후 제거하지 않음. (넥사크로 함수. )
	NewWord := SubStr(NewWord,5)
}else if( SubStr(NewWord,1,4) = "jsc." ){		; 공백이후 제거하지 않음. (자바스크립트 코드)
	NewWord := SubStr(NewWord,5)
}else if( SubStr(NewWord,1,4) = "sql." ){		; 공백이후 제거하지 않음. (SQL 코드 샘플)
	NewWord := SubStr(NewWord,5)
}else if( SubStr(NewWord,1,5) = "java." ){		; 공백이후 제거하지 않음. (코드 템플릿 으로 사용됨)
	NewWord := SubStr(NewWord,6)
}else if( SubStr(NewWord,1,4) = "tbl." ){
	NewWord := SubStr(NewWord,5)
	NewWord := RegExReplace(NewWord,"\s+.*","")
}else if( SubStr(NewWord,1,5) = "code." ){
	NewWord := SubStr(NewWord,6)
	NewWord := RegExReplace(NewWord,"\s+.*","")
}else if( SubStr(NewWord,1,5) = "word." ){
	NewWord := SubStr(NewWord,6)
	NewWord := RegExReplace(NewWord,"\s+.*","")
}else if( SubStr(NewWord,1,8) = "special." ){
	NewWord := SubStr(NewWord,9)
	NewWord := RegExReplace(NewWord,"\s+.*","")
}
